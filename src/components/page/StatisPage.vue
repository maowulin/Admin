<template>
	<div>
		...
		<div class="user_head2">
			...
			<el-date-picker v-model="presentDate" type="daterange" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd"
			 :default-value="defaultDate" @change="dateChange">
		...
		<div v-if="chartShow" class="chart-bord">
			...
			<line-chart v-for="item in chartsGroup" class="line-chart" :key="item.group" :charts-id="item.id" :chart-title="item.chartTitle"
			:chart-unit="item.hcartUnit" :charts-data="item.items" :table-data="tableData"></line-chart>
		...
		<div v-else>
			...
			<egrid class="egrid" fit :key="key" :data="tableData" :columns="newColumns" :columns-schema="columnsSchema" :columns-props="columnsProps"
